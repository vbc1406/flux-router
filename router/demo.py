...
_SAMPLES: list[dict] = [
    ...
    # CHEAP tier targets
    {"prompt": "Classify this review as positive or negative: 'Great product, loved it!'", "priority": "normal"},
    {"prompt": "Extract all email addresses from: john@test.com, jane@corp.io, bob@mail.com", "priority": "normal"},
    {"prompt": f"Summarize this email about a meeting reschedule: {_MOCK_EMAIL}", "priority": "normal"},
    {"prompt": "What is photosynthesis?",                       "priority": "normal"},
    {"prompt": "Convert 100 USD to EUR",                        "priority": "normal"},

    # MID tier targets
    {"prompt": "Write a Python function to reverse a linked list with O(1) space complexity", "priority": "normal"},
    {"prompt": "Compare PostgreSQL vs MongoDB for a real-time analytics dashboard",           "priority": "normal"},
    {"prompt": "Explain the CAP theorem and its implications for distributed databases",       "priority": "normal"},
    {"prompt": "Write a 500-word blog post about the future of renewable energy",              "priority": "normal"},
    {"prompt": "Explain quantum entanglement to a 10 year old using simple analogies",         "priority": "normal"},

    # PREMIUM tier targets
    {"prompt": "Write a React component for a drag-and-drop kanban board with state management, animations, and persistence", "priority": "normal"},
    {"prompt": "Analyze the legal implications of GDPR Article 17 on cross-border data transfers considering recent ECJ rulings", "priority": "normal", "sensitivity": "confidential"},
    {"prompt": f"Debug this async Python program with a subtle deadlock:\n{_MOCK_CODE}",      "priority": "high"},
    {"prompt": "Write a comprehensive REST API with auth, rate limiting, pagination, and WebSocket support in FastAPI", "priority": "normal"},
    {"prompt": "Design a database schema for a multi-tenant SaaS platform with row-level security, audit logging, and soft deletes", "priority": "normal"},

    # ULTRA tier targets
    {"prompt": "Prove that the sum of all natural numbers diverges, then explain why Ramanujan's -1/12 result is valid in zeta function regularization. Show all steps with ∑ notation.", "priority": "critical"},
    {"prompt": f"Given this contract, identify every liability clause, cross-reference with indemnification sections, and produce a risk matrix:\n{_MOCK_CONTRACT}", "priority": "high"},

    # SPECIAL cases
    {"prompt": "Write a story about a time traveler who accidentally prevents their own birth", "priority": "low", "temperature": 1.0},
    {"prompt": "What objects are visible in this image? Describe in detail.", "priority": "normal", "capabilities": ["vision"]},
    {"prompt": "Solve: minimize f(x,y) = x² + y² subject to x + y ≥ 10, using KKT conditions. Show all derivation steps.", "priority": "critical"},
]
...
async def run_demo() -> None:
    ...
    quality     = QualityScorer(adaptive)
    ...
    tier_str = "  |  ".join(
        f"[bold]{t}[/]: {tier_counts[t]}"
        for t in ["free", "cheap", "mid", "premium", "ultra"]
    )
    ...
    console.print(
        f"  [magenta]Tier breakdown[/]:  {tier_str}    "
        f"[yellow]Avg complexity[/]: {avg_score:.3f}"
    )
    console.print(
